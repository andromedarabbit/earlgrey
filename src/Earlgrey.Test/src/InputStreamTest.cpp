...
namespace Earlgrey
{
	namespace Test
	{
		...
		{
		public:
			explicit ConcreteInputStream(std::tr1::shared_ptr<AsyncStream> stream) 
				: InputStream(stream)
			{
			}
		...
		TEST(InputStreamTest, OperatorTest)
		{
			...
			std::tr1::shared_ptr<AsyncStream> stream = std::tr1::shared_ptr<AsyncStream>(new AsyncStream());
