...
namespace Earlgrey
{
	namespace Extension
	{
		...
		{
			EARLGREY_ASSERT(response != NULL && keyword != NULL);
			...
			try
			{
				...
				if(authenticate && IsKeywordSupported(RecvBuf, "AUTH") == true)
				{
					...
					else if(IsKeywordSupported(RecvBuf, "CRAM-MD5") == true)
					{
						...
						size_t passwordLength=m_sPassword.size();
		...
		void CSmtp::SendData(Command_Entry* pEntry)
		{
			...
			size_t idx = 0, res, nLeft = strlen(SendBuf);
			...
			while(nLeft > 0)
			{
				...
				if(res && FD_ISSET(hSocket,&fdwrite))
				{
					int res = send(hSocket, &SendBuf[idx], EARLGREY_NUMERIC_CAST<int>(nLeft), 0);
		...
		{
			size_t offset = 0, nLeft = strlen(SendBuf);
			int res = 0;
			...
			while(nLeft > 0)
			{
				...
				if(FD_ISSET(hSocket,&fdwrite) || (write_blocked_on_read && FD_ISSET(hSocket, &fdread)) )
				{
					...
					/* Try to write */
					res = SSL_write(ssl, SendBuf+offset, EARLGREY_NUMERIC_CAST<int>(nLeft));
